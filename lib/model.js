...
function extractMcClass(classSig, creatorDef) {
	if (classSig[0] != 'assign') return null;
	...
	if (!res) return null;
	var bounds = null;
	if (isArray(creatorDef) && isArray(creatorDef[1])) // bounds
		bounds = extractBounds(creatorDef[1][2]);
	// it's a valid class
	var sig = {
		name: res.join('.'),
		base: 'MovieClip',
...
function parseTopLevel(ast) {
	...
	for(var i=0; i<defs.length; i++) {
		...
		// is it a MovieClip class? (new syntax)
		res = extract(sig, ['stat', 'assign', true, 'dot', '<?>', 'prototype', 'call', '<?>', '<?>']);
		if (res) {
			var newClass = extractMcClass(res[0], res[2]);
			if (newClass != null) {
				curClass = newClass;
				classes.push(newClass);
			}
	...
	if (classes.length > 0) ns = classes[0].name.split('.')[0];
	return {
		classes: classes
	}
}
